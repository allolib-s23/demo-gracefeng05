...
public:
    ...
    void playQuintet() {
        // read json file
        std::ifstream f("/Users/gracefeng/allolib/demo1-gracefeng05/tutorials/synthesis/airSupply.json");
        // cout << "FILE" << endl;
        // cout << f.rdbuf();
        json music = json::parse(f);

        json synth1 = music["tracks"][1]["notes"];
        json synth2 = music["tracks"][2]["notes"];

        auto s1 = synth1.begin();
        auto s2 = synth2.begin();


    while(s1 != synth1.end() || s2 != synth2.end())
    {
        if(s1 != synth1.end())
        {
            auto note = *s1;
